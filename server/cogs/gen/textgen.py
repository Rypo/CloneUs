...
class TextGen(commands.Cog, SetConfig, InitialsMixin):
    ...
    async def switch_model(self, ctx: commands.Context, checkpoint_subdir:str = None, model_runname: str = None, 
        ...
        allmodelpath = settings.RUNS_DIR #/ 'mistral-7b-i4/'
        model_outpaths = sorted([p.parent for p in allmodelpath.rglob('config.yaml') if any(p.parent.glob('checkpoint*'))])
        model_basedir = None
        # None passed
        if model_runname is None and checkpoint_subdir is None: 
            if dtype is not None or attn_implementation is not None:
                await ctx.defer()
                await self.txtdown(ctx, False, False)
                
                await self.clomgr.load(self.clomgr.config_file.parent, self.clomgr.ckpt_subdir, dtype=dtype, attn_implementation=attn_implementation)
            ...
            ppview = cview.PrePagedView(self.clomgr.modelview())
            await ppview.send(ctx, 'All Valid Model Options')
            return

        # checkpoint passed
        if checkpoint_subdir:
            if not checkpoint_subdir.startswith('checkpoint-'):
                checkpoint_subdir = 'checkpoint-'+checkpoint_subdir
            if model_runname is None: 
                model_basedir = self.clomgr.mdir_comps.basedir_path
        ...
        if model_basedir is None: 
            ...
            if not matches:
                ppview = cview.PrePagedView(self.clomgr.modelview(remove_empty=True))
                await ppview.send(ctx, f'No match for "{model_runname}"')
                return
            
            if len(matches)>1:
                ppview = cview.PrePagedView(self.clomgr.modelview(modelname_filter=model_runname, remove_empty=True))
        ...
        if checkpoint_subdir is None:
            emb=discord.Embed(title='Options', description=io_utils.find_checkpoints(model_basedir, md_format=True, wrapcodeblock=True))
            await ctx.send('Active Model Checkpoints', embed=emb)
            return 

        full_model_path = (model_basedir/checkpoint_subdir)
        ...
        await self.txtdown(ctx, False, False)
        await self.clomgr.load(model_basedir, checkpoint_subdir, dtype=dtype, attn_implementation=attn_implementation)
        await self.txtup(ctx, False, False)
        #await self.status_report(ctx) # TODO: FIX
