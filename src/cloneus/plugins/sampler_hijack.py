# Adapted from: https://github.com/oobabooga/text-generation-webui/blob/dev/modules/sampler_hijack.py
...
class XTCLogitsWarper(LogitsWarper):
    def __init__(self, threshold: float, probability: float, filter_value: float = -float("Inf"), _tokenizer=None):
        ...
        if _tokenizer.eos_token_id is not None:
            self.special_token_ids.append(_tokenizer.eos_token_id)
...
# def get_logits_processor_patch(tokenizer):
#     #tokenizer = kwargs.pop('tokenizer', None)
#     return functools.partial(_get_logits_processor_patch, tokenizer=tokenizer)#, **kwargs)

def get_logits_processor_patch(self, **kwargs):
    tokenizer = TOKENIZER
    #tokenizer = kwargs.pop('tokenizer', None)
    # kwargs: dict_keys(['generation_config', 'input_ids_seq_length', 'encoder_input_ids', 'prefix_allowed_tokens_fn', 'logits_processor', 'device', 'model_kwargs', 'negative_prompt_ids', 'negative_prompt_attention_mask'])
    # model_kwargs: dict_keys(['attention_mask', 'use_cache', 'past_key_values'])
    ...
    warpers_with_tokenizer = LogitsProcessorList()
    ...
    if tokenizer is not None:
        warpers += warpers_with_tokenizer
    # Sort the samplers.
...
# 4.45.0 migration: https://github.dev/huggingface/transformers/blob/70d5df61079c50c770347a71b42b518e0fe4d0ff/src/transformers/generation/utils.py#L358
